...
| `name`                | 書き: `CreateItemRequest.name`, `UpdateItemRequest.name`<br>読み: `ItemResponse.name`     | `ItemMapper.toResponse`, `ItemMapper.updateEntity` | 作成時は `ItemCommandService` が `trim()` と未完了初期化を担当。`quantified` の場合は `ItemQuantified.name` と同期する。 |
...
| `category`            | 書き: `CreateItemRequest.category`, `UpdateItemRequest.category`<br>読み: `ItemResponse.category` | `ItemMapper.toResponse`, `ItemMapper.updateEntity` | plain / quantified 共通の分類。`generated + auto` は生成ルールから決定できる。 |
...
| `quantified`          | 書き: `CreateItemRequest.quantified`, `UpdateItemRequest.quantified`<br>読み: `ItemResponse.quantified` | `ItemMapper.toQuantifiedEntity`, `ItemMapper.updateQuantifiedEntity`, `ItemMapper.toQuantifiedResponse` | `ItemQuantified` と 1:1。`plain` の場合は `null`。 |
...
| `name`                 | 書き: `QuantifiedItemRequest.name`<br>読み: `QuantifiedItemResponse.name`       | `ItemMapper.updateQuantifiedEntity`, `ItemMapper.toQuantifiedResponse` | 数量付き item の構造化された品目名。`item.name` と backend で同期する。 |
