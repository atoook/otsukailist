...
| `name`                | 書き: `CreateItemRequest.name`, `UpdateItemRequest.name`<br>読み: `ItemResponse.name`     | `ItemMapper.toResponse`, `ItemMapper.updateEntity` | 作成時は `ItemCommandService` が `trim()` と未完了初期化を担当。`plain` / `quantified` 共通の唯一の名称フィールド。 |
