...
このドキュメントは、item 完了更新を意図別 API として扱うための設計方針を定義する。現行実装では item `version`、`If-Match`、`changed` を含むレスポンス DTO、`precondition_required` / `precondition_failed` エラーコードと例外ハンドラを導入済み。
...
競合制御の全体方針は [競合制御方針](../architecture/concurrency-control-policy.md) に従う。item完了更新も item `version` + `If-Match` による楽観ロックを使い、stale state の場合は `412 precondition_failed` を返す。
...
`PATCH /api/lists/{listId}/items/{itemId}` の `completed` 更新は既存互換として残すが、一覧のチェック操作では使わない。item完了APIには画面で保持していた item `version` を `If-Match` で送る。`412` の場合は snapshot を取り直し、最新状態を表示する。
